...
void state_off() {
	...
	if (first){
		for (int i = 0; i < 1; i++) { tone(BUZZER, 1000, 300); delay(300); noTone(BUZZER); delay(100); }
...
void state_on() {
	...
	boolean guard =  millis() - time > debounce;
	if (first){
		for (int i = 0; i < 2; i++) { tone(BUZZER, 1000, 300); delay(300); noTone(BUZZER); delay(100); }
		for (int i = 0; i < 1; i++) { tone(BUZZER, 1000, 1000); delay(1000); noTone(BUZZER); delay(100); }
		first = 0;
	}
	if (digitalRead(BUTTON) == HIGH && guard) {
		first = 1; time = millis(); state_off();
	} else {
		state_on();
	}
}
